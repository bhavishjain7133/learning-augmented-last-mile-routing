...
from lastmile.experiments import (
    evaluate_geodesic_screen,
    select_preference_weight,
    validation_summary,
)
from lastmile.model import ZoneTransitionModel
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the leakage-safe geodesic screening experiment")
    ...
    model = ZoneTransitionModel(
        ...
        use_parent_zones=False,
    ).fit(split.train)

    validation_routes = split.validation[: args.validation_limit]
    validation = evaluate_geodesic_screen(
        validation_routes,
        model,
        config["model"]["preference_weight_grid"],
    )
    summary = validation_summary(validation)
    tolerance = config["model"].get("median_cost_tolerance", 0.05)
    selected_weight = select_preference_weight(summary, tolerance)

    test_routes = split.test[: args.test_limit]
    test = evaluate_geodesic_screen(test_routes, model, [selected_weight])

    output = ROOT / "artifacts" / "geodesic_screen"
    ...
    metadata = {
        ...
        "screening_note": (
            "Geodesic screening only. Supplied travel-time and official-score "
            "experiments remain confirmatory."
        ),
    }
